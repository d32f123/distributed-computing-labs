...
void broadcast_started_msg(Communicator* comm, balance_t balance)
{
    ...
    // set string
    int length = snprintf(buffer, BUFFER_SIZE, log_started_fmt, get_physical_time(), comm->curr_id, getpid(), getppid(), balance);
    if (length <= 0)
    {
...
void broadcast_done_msg(Communicator* comm, balance_t balance)
{
    ...
    // set string
    int length = snprintf(buffer, BUFFER_SIZE, log_done_fmt, get_physical_time(), comm->curr_id, balance);
    if (length <= 0)
    {
        perror("failed to apply snprintf");
        exit(-2);
