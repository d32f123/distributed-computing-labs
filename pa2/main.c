...
void update_history(Communicator* comm, BalanceHistory* history, BalanceState* state, balance_t sum)
{
    static timestamp_t prevTime = 0;
    timestamp_t currTime = get_physical_time();
...
void do_transfer(Communicator* comm, Message* msg, TransferOrder* order, BalanceState* balanceState, BalanceHistory* balanceHistory)
{
    ...
    if (order->s_src == comm->curr_id)
    {
        ...
        update_history(comm, balanceHistory, balanceState, -order->s_amount);
