...
{
    fprintf(stdout, log_started_fmt, lid, getpid(), getppid());
    fprintf(events_log_file, log_started_fmt, lid, getpid(), getppid());
}

void log_received_all_started(local_id lid)
{
    fprintf(stdout, log_received_all_started_fmt, lid);
    fprintf(events_log_file, log_received_all_started_fmt, lid);
}

void log_done(local_id lid)
{
    fprintf(stdout, log_done_fmt, lid);
    fprintf(events_log_file, log_done_fmt, lid);
}

void log_received_all_done(local_id lid)
{
    fprintf(stdout, log_received_all_done_fmt, lid);
    fprintf(events_log_file, log_received_all_done_fmt, lid);
}
